...
serve(async (req) => {
  ...
  try {
    ...
    // IMPORTANT: Deno's WebSocket doesn't support custom headers
    // We MUST use a workaround - for now, log error and guide user to alternative approach
    console.error("CRITICAL: Cannot connect to OpenAI Realtime API from Deno Edge Function");
    console.error("Reason: Deno WebSocket API doesn't support custom headers (Authorization)");
    console.error("Solution: Need to implement client-side direct connection with server-provided token");

    throw new Error("WebSocket with custom headers not supported in Deno - architecture change needed");
